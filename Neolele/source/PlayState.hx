...
{
	var SECRET:String = 'Hey look! You found an easter egg!';
	...
	private var thornRingBf:Bool = false;
	private var thornRingDad:Bool = false;
	...
	public static var secretSong:Bool = false;

	public static var isIceshock:Bool = false;
	...
	override public function create()
	{
		...
		switch (SONG.song.toLowerCase())
		{
			...
			case 'girl-next-door':
				dialogue = CoolUtil.coolTextFile(Paths.txt('girl-next-door/dialogue'));
			case 'rude-buster':
				dialogue = CoolUtil.coolTextFile(Paths.txt('rude-buster/dialogue'));
			case 'ultra-hd':
				dialogue = CoolUtil.coolTextFile(Paths.txt('ultra-hd/Caught In 4K'));
			case 'iceshock':
				dialogue = CoolUtil.coolTextFile(Paths.txt('iceshock/dialogue'));
		}
		...
		switch(SONG.song.toLowerCase())
		{
			...
			{	
					defaultCamZoom = 0.9;
					curStage = 'light world';
					var bg:FlxSprite = new FlxSprite();
					bg.loadGraphic(Paths.image('deltarune/noelleBG'));
					bg.antialiasing = true;
					bg.screenCenter();
					bg.scrollFactor.set(0.9, 0.9);
					bg.active = false;
			...
			{
					defaultCamZoom = 0.9;
					curStage = 'castle town';
					var bg:FlxSprite = new FlxSprite();
					bg.loadGraphic(Paths.image('deltarune/castle-town'));
					bg.antialiasing = true;
					bg.screenCenter();
					bg.scrollFactor.set(0.9, 0.9);
					bg.active = false;
					...
					trace('snowgrave fnf refence?!?!?!?!?!1111??!11111');
			}
			case 'anadila':
		    {
	                defaultCamZoom = 0.2;
					...
					bg.loadGraphic(Paths.image('deltarune/amogus'));
	                bg.antialiasing = true;
	                bg.scrollFactor.set(0.9, 0.9);
	                bg.active = false;
	                add(bg);

					FlxG.save.data.anadila = true;
			...
			case 'ultra-hd':
			{
					defaultCamZoom = 0.6;
					curStage = 'hentaiinc';
					var bg:FlxSprite = new FlxSprite(-1500, -700).loadGraphic(Paths.image('deltarune/Hentai-Inc'));
					bg.antialiasing = true;
					bg.scrollFactor.set(0.9, 0.9);
					bg.active = false;
					add(bg);

					FlxG.save.data.ultraHD = true;
		...
		switch (SONG.player2)
		{
			...
			case 'spirit':
				...
				camPos.set(dad.getGraphicMidpoint().x + 300, dad.getGraphicMidpoint().y);
			case 'susie':
				dad.setGraphicSize(Std.int(dad.height / 2.5));
				dad.y -= 300;
			case 'troll':
				dad.x -= 20;
				dad.y -= 100;
				dad.setGraphicSize(Std.int(dad.height / 1.25));
		}
		...
		if (isStoryMode)
		{
			switch (curSong.toLowerCase())
			{
				...
				case 'senpai' | 'roses' | 'thorns':
					schoolIntro(doof);
				case 'girl-next-door' | 'rude-buster' | 'iceshock' | 'ultra-hd':
					schoolIntro(doof);
				default:
					startCountdown();
			}
	...
	function schoolIntro(?dialogueBox:DialogueBox):Void
	{
		...
		if (SONG.song.toLowerCase() == 'roses' || SONG.song.toLowerCase() == 'thorns' || SONG.song.toLowerCase() == 'girl-next-door')
		{
			remove(black);
	...
	override public function update(elapsed:Float)
	{
		...
		if (FlxG.keys.justPressed.ENTER && startedCountdown && canPause)
		{
			...
			var pauseChance = FlxG.random.int(0, 99);
		...
		if (health <= 0)
		{
			...
			if (SONG.song.toLowerCase() == 'iceshock')
				isIceshock = true;
	...
	*/
	...
	override function beatHit()
	{
		...
		if (thornRingBf && healthBar.percent >= 33)
		{
			health -= 0.05;
		}

		if (thornRingDad && healthBar.percent <= 66)
		{
			health += 0.05;
		}
	}
